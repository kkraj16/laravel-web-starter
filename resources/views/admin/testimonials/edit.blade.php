        ...
        <form action="{{ route('admin.testimonials.update', $testimonial) }}" method="POST">
            ...
            <div class="card card-outline card-warning">
                <div class="card-header">
                    <h3 class="card-title fw-bold">Edit Review #{{ $testimonial->id }}</h3>
                ...
                <div class="card-body">
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            ...
                            <input type="date" name="review_date" id="review_date" class="form-control @error('review_date') is-invalid @enderror" value="{{ old('review_date', $testimonial->review_date->format('Y-m-d')) }}" required>
                            @error('review_date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="content" id="content" rows="4" class="form-control @error('content') is-invalid @enderror" required>{{ old('content', $testimonial->content) }}</textarea>
                        @error('content')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
